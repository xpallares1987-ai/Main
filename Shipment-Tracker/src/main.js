...
window.setLanguage = (lang) => { I18nService.setLang(lang); };
window.showDetails = (id) => {
    const shipment = state.allShipments.find(s => s.id === id);
    if (shipment)
        ModalUI.open(shipment);
};
window.closeModal = () => { ModalUI.close(); };
...
function updateStaticTranslations() {
    ...
    if (els.live)
        els.live.textContent = t.liveData;
    if (els.search)
        els.search.placeholder = t.searchPlaceholder;
    if (els.export)
        els.export.textContent = t.exportReport;
    if (els.chartStatus)
        els.chartStatus.textContent = t.statusDist;
    if (els.chartMode)
        els.chartMode.textContent = t.modeDist;
    if (els.navDash)
        els.navDash.textContent = t.dashboard;
    if (els.navCont)
        els.navCont.textContent = t.contacts;
    if (els.agentsTitle)
        els.agentsTitle.textContent = t.agents;
    const filterSelect = els.filter;
    if (filterSelect) {
...
function updateView() {
    ...
    if (statsPanel)
        statsPanel.innerHTML = UIComponents.renderStats(filtered);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const navCont = document.getElementById('nav-contacts');
    if (searchInput)
        searchInput.value = state.filters.term;
    if (statusFilter)
    ...
    btnAnalytics?.addEventListener("click", () => {
        ...
        if (panel)
            panel.style.display = state.showAnalytics ? 'grid' : 'none';
    });
